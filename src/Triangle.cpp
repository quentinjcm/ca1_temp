...
bool Triangle::intersect(const Ray &_ray, IsectData *_intersection)
{
   ...
   * Then the vector m_v1->p is written prametrically in terms of the
   * vectors alpha(m_v1->m_v2) and beta(m_v1->m_v3). If alpha > 0 and
  ...
  if (denominator){
    ...
    //[u0, v0] = vector from m_v1 to intersection point
    float u0 = p[m_i1] - m_v1[m_i1];
    float v0 = p[m_i2] - m_v1[m_i2];
    //[u1, v1] = vector from m_v1 to m_v2
    float u1 = m_v2[m_i1] - m_v1[m_i1];
    float v1 = m_v2[m_i2] - m_v1[m_i2];
    //[u2, v2] = vector from m_v1 to m_v3
    float u2 = m_v3[m_i1] - m_v1[m_i1];
    float v2 = m_v3[m_i2] - m_v1[m_i2];
